...
// ─── Types ────────────────────────────────────────────────────────────────────
type Tab = { id: number; label: string; icon: string; color: string; accent: string };

// ─── Constants ────────────────────────────────────────────────────────────────
const TABS: Tab[] = [
  { id: 0, label: "Transformer", icon: "⚡", color: "#2563eb", accent: "#dbeafe" },
  { id: 1, label: "Solar PV", icon: "☀", color: "#d97706", accent: "#fef3c7" },
  { id: 2, label: "Wind", icon: "🌬", color: "#7c3aed", accent: "#ede9fe" },
  { id: 3, label: "EV Network", icon: "⚙", color: "#059669", accent: "#d1fae5" },
];
...
function TransformerSchematic() {
  return (
    <svg viewBox="0 0 460 220" fill="none" xmlns="http://www.w3.org/2000/svg" className="w-full">
      ...
      <text x="28" y="165" fill="#2563eb" fontSize="7" fontFamily="monospace" fontWeight="600">■ AI ANALYSIS</text>
...
export default function HeroSection() {
  ...
  return (
    ...
    >
      {/* Ambient blobs */}
      <m.div className="absolute pointer-events-none" style={{ x: blobX, y: blobY, top: "-20%", right: "-10%", width: "60vw", height: "60vw", borderRadius: "40% 60% 70% 30% / 30% 40% 60% 70%", background: "radial-gradient(circle, rgba(37,99,235,0.07) 0%, transparent 70%)" }} />
      <m.div className="absolute pointer-events-none" style={{ bottom: "-15%", left: "-10%", width: "50vw", height: "50vw", borderRadius: "60% 40% 30% 70% / 60% 30% 70% 40%", background: "radial-gradient(circle, rgba(124,58,237,0.05) 0%, transparent 70%)" }} />
      ...
      {/* ── HERO CONTENT ── */}
      <div className="relative z-10 max-w-[1320px] mx-auto px-6 lg:px-12 pt-16 pb-20">

        {/* Badge */}
        <m.div variants={fadeUp} custom={0} initial="hidden" animate="show" className="mb-10">
          <span className="inline-flex items-center gap-2.5 px-4 py-2 rounded-full text-[11px] font-semibold tracking-wider uppercase"
        ...
        <div className="grid lg:grid-cols-[1fr_1fr] gap-12 lg:gap-20 items-center">
          ...
          <div>
            {/* Headline */}
            <m.div variants={stagger} initial="hidden" animate="show" className="mb-7" style={{ perspective: 1200 }}>
              {[
                [{ t: "India's", a: false }, { t: "Intelligent", a: true }],
                [{ t: "Energy", a: false }, { t: "Grid", a: false }],
                [{ t: "AI", a: true }, { t: "Platform", a: false }],
              ].map((line, li) => (
                <div key={li} className="flex flex-wrap gap-x-3 leading-none mb-1">
                  {line.map(({ t, a }, wi) => (
            ...
            <m.p variants={fadeUp} custom={0.5} initial="hidden" animate="show"
              ...
              style={{ color: "#64748b", maxWidth: 440, fontFamily: "'DM Sans', sans-serif", fontSize: "17px", lineHeight: 1.7 }}>
            ...
            {/* CTAs */}
            <m.div variants={fadeUp} custom={0.6} initial="hidden" animate="show" className="flex flex-wrap gap-3 mb-12">
              <MagneticBtn
                href="#platform"
                className="inline-flex items-center gap-2 px-7 py-3.5 rounded-xl font-semibold text-sm"
                style={{ background: "#2563eb", color: "white", fontFamily: "'DM Sans', sans-serif", textDecoration: "none", boxShadow: "0 8px 32px rgba(37,99,235,0.3)" }}>
                Explore Platform
              ...
              <MagneticBtn
                href="#demo"
                className="inline-flex items-center gap-2 px-7 py-3.5 rounded-xl font-semibold text-sm"
                style={{ background: "white", color: "#0f172a", fontFamily: "'DM Sans', sans-serif", textDecoration: "none", border: "1.5px solid #e2e8f0", boxShadow: "0 2px 8px rgba(0,0,0,0.06)" }}>
                Request Demo
              </MagneticBtn>
            </m.div>
            ...
            <m.div variants={fadeUp} custom={0.7} initial="hidden" animate="show"
              ...
              {STATS.map(({ value, label, color }) => (
                <div key={label} className="p-4 rounded-xl" style={{ background: "white", border: "1px solid #f1f5f9", boxShadow: "0 1px 4px rgba(0,0,0,0.04)" }}>
                  <div style={{ fontFamily: "'Syne', sans-serif", fontWeight: 800, fontSize: "22px", color, lineHeight: 1 }}>{value}</div>
                  <div style={{ fontFamily: "'DM Sans', sans-serif", fontSize: "11px", color: "#94a3b8", marginTop: 4 }}>{label}</div>
                </div>
              ))}
            </m.div>
          </div>
          ...
          <m.div variants={fadeUp} custom={0.3} initial="hidden" animate="show">
            <div className="rounded-2xl overflow-hidden" style={{ background: "white", border: "1.5px solid #e2e8f0", boxShadow: "0 24px 80px rgba(0,0,0,0.08)" }}>
              ...
              {/* Schematic canvas */}
              <div className="relative" style={{ minHeight: 220, padding: "12px 8px 4px" }}>
              ...
              <div className="grid grid-cols-4 border-t" style={{ borderColor: "#f1f5f9" }}>
                {METRICS[activeTab].map(({ label, unit }, i) => (
                  <div key={i} className="p-3 border-r last:border-r-0" style={{ borderColor: "#f1f5f9" }}>
                    <div style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: "8px", color: "#94a3b8", textTransform: "uppercase", marginBottom: 3 }}>{label}</div>
                    <AnimatePresence mode="wait">
                      <m.div key={metrics[i]}
                        initial={{ opacity: 0, y: 4 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -4 }}
                        style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: "14px", fontWeight: 600, color: "#0f172a" }}>
                        {metrics[i]}<span style={{ fontSize: "9px", color: "#94a3b8", fontWeight: 400 }}>{unit && ` ${unit}`}</span>
                      </m.div>
                    </AnimatePresence>
                    <div style={{ fontSize: "9px", color: "#059669", marginTop: 2 }}>● Live</div>
                  </div>
                ))}
              </div>
            ...
            <m.div variants={fadeUp} custom={0.5} initial="hidden" animate="show"
              className="flex flex-wrap items-center gap-3 mt-5 justify-end">
              {["Make in India", "Patent No. 202521117118", "MSME Certified"].map((item) => (
                <span key={item} style={{
                  ...
                  padding: "4px 10px", borderRadius: 99, background: "white",
        ...
        {/* ── SECTOR CARDS ── */}
        <m.div initial={{ opacity: 0, y: 40 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.8 }} className="mt-28 mb-10">
          <p style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: "11px", color: "#2563eb", marginBottom: 8 }}>// SECTORS WE COVER</p>
          <h2 style={{ fontFamily: "'Syne', sans-serif", fontWeight: 800, fontSize: "clamp(28px, 4vw, 52px)", color: "#0f172a", lineHeight: 1.05 }}>
            Every Node. Every Watt.<br /><span style={{ color: "#2563eb" }}>Monitored.</span>
          </h2>
          <p style={{ color: "#64748b", fontSize: "16px", lineHeight: 1.7, maxWidth: 480, marginTop: 12 }}>
            From the transformer at your substation to the solar panel on your rooftop — CyboKrafts watches everything and predicts failures before they happen.
          </p>
        </m.div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
          {[
            ...
            <m.div key={i}
              ...
              <div style={{ fontFamily: "'Syne', sans-serif", fontWeight: 700, fontSize: "16px", color: "#0f172a", marginBottom: 6 }}>{s.name}</div>
        ...
        <div className="mt-28">
          <m.div initial={{ opacity: 0, y: 24 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.8 }} className="mb-12">
            <p style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: "11px", color: "#2563eb", marginBottom: 8 }}>// IoT ARCHITECTURE</p>
            <h2 style={{ fontFamily: "'Syne', sans-serif", fontWeight: 800, fontSize: "clamp(24px, 3.5vw, 48px)", color: "#0f172a", marginBottom: 8 }}>The Intelligence Stack</h2>
            <p style={{ color: "#94a3b8", fontFamily: "'JetBrains Mono', monospace", fontSize: "11px" }}>Sensor → Decision in &lt; 50ms</p>
          </m.div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-4 gap-0 rounded-2xl overflow-hidden" style={{ border: "1.5px solid #e2e8f0" }}>
            {[
              ...
              <m.div key={i}
                ...
                className="p-6 lg:border-r last:border-r-0 border-b lg:border-b-0"
                style={{ borderColor: "#e2e8f0", background: "white" }}>
                <div style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: "9px", color: "#94a3b8", marginBottom: 8 }}>{layer.layer}</div>
                <div style={{ fontFamily: "'Syne', sans-serif", fontWeight: 700, fontSize: "17px", color: "#0f172a", marginBottom: 6 }}>{layer.name}</div>
                <div style={{ fontSize: "12px", color: "#64748b", lineHeight: 1.65, marginBottom: 12 }}>{layer.detail}</div>
                <div style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: "10px", fontWeight: 600, color: layer.color }}>{layer.version}</div>
                {/* Flow dot */}
                {i < 3 && (
                  <div className="hidden lg:block absolute right-0 top-1/2 -translate-y-1/2 w-3 h-3 rounded-full z-10" style={{ background: layer.color, right: -6 }} />
                )}
              </m.div>
            ))}
          </div>
